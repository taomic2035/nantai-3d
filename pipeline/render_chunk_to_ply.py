...
def _emit_road(road, x_offset: int, y_offset: int) -> np.ndarray:
    ...
    for i in range(len(road.points) - 1):
        ...
        t = np.random.default_rng(hash(road.id + str(i)) & 0xFFFFFFFF)
        ...
        pts_list.append(pts)
    if not pts_list:
        return np.zeros(0, dtype=pts_list[0].dtype)
    return np.concatenate(pts_list)


def _emit_building(b, x_offset: int, y_offset: int) -> np.ndarray:
    """建筑: 盒子墙 + 屋顶"""
    bx, by = b.pos
    bx += x_offset
    by += y_offset
    ...
    for ia, ib in wall_edges:
        p0 = corners[ia]
        p1 = corners[ib]
        t = np.random.default_rng(hash(b.id + str(ia)) & 0xFFFFFFFF)
    ...
    roof_top[:, 2] += h_roof / 2
    t = np.random.default_rng(hash(b.id + "roof") & 0xFFFFFFFF)
...
def _emit_vegetation(veg, x_offset: int, y_offset: int) -> np.ndarray:
    ...
    t = np.random.default_rng(hash(veg.id) & 0xFFFFFFFF)
...
def _emit_water(water, x_offset: int, y_offset: int) -> np.ndarray:
    ...
    for i in range(len(water.points) - 1):
        ...
        t = np.random.default_rng(hash(water.id + str(i)) & 0xFFFFFFFF)
        u = t.uniform(0, 1, n_per_seg)
        v = t.uniform(-w / 2, w / 2, n_per_seg)
        base = p0[None] + u[:, None] * seg[None]
        offset = v[:, None] * perp[None]
...
def render_chunk_to_ply(layout: ChunkLayout, out_path: Path) -> int:
    ...
    for b in layout.buildings:
        layers.append(_emit_building(b, x_offset, y_offset))
    ...
    all_pts = np.concatenate([l for l in layers if len(l) > 0])
    n = len(all_pts)
...
def render_chunkset(
    ...
    for cx in range(x_min, x_max):
        for cy in range(y_min, y_max):
            ...
            n = render_chunk_to_ply(layout, ply_path)
