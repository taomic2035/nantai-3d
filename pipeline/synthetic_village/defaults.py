...
def _read_json(path: Path) -> bytes:
    """Read a default resource JSON file through a single O_NOFOLLOW descriptor.

    Pre-checks with ``lstat`` to reject symlinks and Windows reparse points
    and to bound the read by ``MAX_DEFAULT_RESOURCE_BYTES``.  The actual
    content read goes through the SAME descriptor, with ``fstat`` identity
    checks before and after reading and a post-read ``lstat`` recheck,
    eliminating the check-then-reopen TOCTOU window of ``stat`` + ``open``.
    """
    try:
        before = path.lstat()
    except FileNotFoundError as exc:
        raise ValueError(f"default resource is absent: {path.name}") from exc
    except OSError as exc:
        raise ValueError(f"default resource cannot be inspected: {path.name}") from exc
    if (
        stat.S_ISLNK(before.st_mode)
        or not stat.S_ISREG(before.st_mode)
        or before.st_size <= 0
        or before.st_size > MAX_DEFAULT_RESOURCE_BYTES
    ):
        raise ValueError(f"default resource is not a bounded regular file: {path.name}")
    flags = os.O_RDONLY | getattr(os, "O_BINARY", 0) | getattr(os, "O_NOFOLLOW", 0)
    try:
        descriptor = os.open(path, flags)
    except OSError as exc:
        raise ValueError(f"default resource cannot be opened: {path.name}") from exc
    payload = bytearray()
    try:
        ...
        if (
            ...
            raise ValueError(
                f"default resource changed before read: {path.name}"
            )
    ...
    try:
        with stream:
            for chunk in iter(lambda: stream.read(1024 * 1024), b""):
                ...
                if len(payload) > MAX_DEFAULT_RESOURCE_BYTES:
                    raise ValueError(
                        f"default resource exceeds bounded read limit: {path.name}"
                    )
            read_fstat = os.fstat(stream.fileno())
    except OSError as exc:
        raise ValueError(f"default resource cannot be read: {path.name}") from exc
    try:
        after = path.lstat()
    except OSError as exc:
        raise ValueError(
            f"default resource cannot be reinspected: {path.name}"
        ) from exc
    if (
        ...
        raise ValueError(f"default resource changed during read: {path.name}")
    raw = bytes(payload)
    json.loads(
        raw.decode("utf-8"),
