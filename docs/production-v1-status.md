...
| 远程 readiness | `preflight-remote` 在 SSH 前复验 canonical runtime policy 及 config 绑定，并在远程 probe 前后检测 policy 漂移；`train-production` 强制消费同一份 ready report，并在 bundle/submit 前复核 config identity、本地输入 SHA 与 runtime policy |
