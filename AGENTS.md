...
- GLM P1 cloud caller 的 89 个聚焦测试已绿，但真实 runner 仍有 config 必然 drift、
  参数未传入 `ns-train`、预处理失败无 receipt 与无 non-mock
  `training_allowed=true` canary 等未决项。详见
  `handoff/REVIEW-CODEX-023-glm-p1-callers.md`；不得宣称真实 cloud caller 或项目已完成。
