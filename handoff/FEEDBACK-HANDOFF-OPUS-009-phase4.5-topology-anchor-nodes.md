...
## Phase 4.5.2 — forest/lower-valley 重定位（已交付）

### 方案

重定位 `_DEFAULT_MODULE_BASE_POSITION` 中的 forest 和 lower-valley 的 base_y，
使候选相机落入现有 path 30m 内。forest 复用现有 `upper-ground-west`（loop
ground node），lower-valley 新增 `valley-ground-001`。

### 修改

| Module | base_y 旧 | base_y 新 | 候选 y | 最近 ground node | 3D 距离 |
|---|---:|---:|---:|---|---:|
| forest-orchard-boundary | 80.0 | 30.0 | 97.5 | upper-ground-west (145, 104) | 28.1 m |
| lower-valley-uphill | 60.0 | -127.5 | -42.5 | valley-ground-001 (-90, -35) | 9.6 m |

新增 `valley-ground-001` 在 (-90, -35) on path-network-002（path-network-002
折线顶点）。

### 全部 6 role 距离验证

| Role | 候选 position | 最近 ground node | 3D 距离 |
|---|---|---|---:|
| central-courtyard-downhill | (25, 40, 79.4) | central-ground-east | 14.9 m |
| bridge-deck-crossing | (-150, -87.5, 56.6) | bridge-ground-001 | 18.7 m |
| watermill-tailrace | (-180, -102.5, 53.6) | watermill-ground-001 | 11.8 m |
| covered-gallery-underpass | (60, 20, 79.6) | gallery-ground-001 | 23.3 m |
| forest-orchard-boundary | (120, 97.5, 76.6) | upper-ground-west | 28.1 m |
| lower-valley-uphill | (-90, -42.5, 56.6) | valley-ground-001 | 9.6 m |
...
| 身份 | 新 SHA-256 |
|---|---|
| elevated_topology_sha256 | `9d9d70b493d4a84021f4219e1c5b2e1ac06dd6402d479448b0c142dd3ce96358` |
| reciprocal_route_module_plan_sha256 | `09f9d520957d23781f9ee206b4430e4a3f0868ac7f14512a46e90d582965520e` |

node_count = 12（8 原有 + 4 anchor）；ground_count = 8（4 loop + 4 anchor）；
`ground_attachment_count` 仍为 4。

### 测试

- 8 TDD passed（elevated_topology，含 1 新增 anchor 测试更新为 4 anchor）
- 262 passed（elevated_topology + reciprocal_route_module + runtime +
  production_profile + environment_module + environment_module_runtime）
- ruff clean

### Codex caller 接入

Phase 4.5.2 完成后，**全部 6 个 role 都已解除 30m 阻塞**。Codex 可以对每个
role 跑 fresh build + preflight + 六层 + post-render v2 + target visibility。

注意：forest/lower-valley 的 module base position 变化会改变几何，需要 Codex
跑 fresh 218-root build + mesh probe 验证 module-module 和 module-environment
intersection 仍然通过。
