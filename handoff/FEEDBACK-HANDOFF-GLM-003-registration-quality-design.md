...
- **Follow-up A**（GLM lane，Codex review 后）：修改 `colmap_register()` 调用
  ...
  **Update (2026-07-23)**：未直接修改 `colmap_register()`。改为通过独立的
  `scripts/emit_registration_quality.py` 工具产出 quality report（设计选择：
  quality report 是独立 artefact，不嵌入 registration wrapper）。`SparseModelEnumeration`
  已在 `pipeline/registration_quality.py` 中实现并测试。`colmap_register()` 仍硬编码
  `sparse/"0"`（行 589-597），但这是 registration 输出路径，不是 quality 判定路径。
- **Follow-up B**（Codex lane）：Studio/Viewer 消费 `RegistrationQualityReport` 显示三态
  **Update (2026-07-23)**：未开始。仍为 Codex lane。
- **Follow-up C**（GLM lane）：`scripts/prepare_import.py` / `reconstruct_local.py` 在进入
  训练前检查 `training_allowed`
  **Update (2026-07-23)**：✅ 完成。`prepare_import.py` 已集成 `--registration-quality-report`
  + `--registration-json` + `--registration-quality-policy`（三参数成对必填），
  `training_allowed=True` 时产出 trusted prefix `training_provenance.v1=<result_sha>`。
...
**Update (2026-07-23)**：Task 1/2/3 均已实现并通过 Codex review。详见
`handoff/REVIEW-CODEX-022-glm-registration-training-trust-contracts.md`（P0 findings）
和 `handoff/FEEDBACK-HANDOFF-GLM-006-review-codex-023-fixes.md`（P1 caller 修复）。
Task 3 实现见 `docs/superpowers/plans/2026-07-23-cloud-training-provenance.md`（Follow-up A/B 已完成，C 仍为 Codex lane）。
