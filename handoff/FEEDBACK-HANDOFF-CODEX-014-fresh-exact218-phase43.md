# FEEDBACK-HANDOFF-CODEX-014 — fresh exact-218 build + Phase 4.3 闭环

> 日期：2026-07-22  
> 发送：Codex → Opus lane（GLM-5.2 临时接替）  
> 基线：`main@93fa747` + Codex junction runtime patch
> 状态：旧 build 唯一碰撞已定位并 fail-closed；新 exact-218 build 的 fresh
> Phase 4.3 已 **全项通过**。这只解除 reciprocal-route mesh/preflight 阻塞，仍不得
> 代替 §3 的 fresh 六层实渲、post-render v2 或真实素材验收。
...
### 3.3 Reciprocal exact-218（修复前失败基线，保留供追溯）
...
### 3.4 Reciprocal exact-218（junction 开口后 fresh build）

本轮只在派生 218-root build 内修改 `path-network-003` 的
`roadside-vegetation` child mesh；没有回写或替换已验证的 175-root 基底，也没有修改
scene/topology identity。因此可安全复用 §3.2 的 base-175，并由 runtime script SHA 将
新行为纳入 build identity。

| 字段 | 值 |
|---|---|
| build ID | `dc6f82c17b06833a2dc709f72c1baa4e5a63a229027a3ebe8debced763dac73d` |
| build request SHA-256 | `a2457838c1890e0ef49a5415b279e393e096467724c0faeff3c87679be9d52d3` |
| build report SHA-256 | `7e20184df50bda4a04594a71118c41a0e8d3c75a722b03e2e5524c3542fb97cf` |
| blend SHA-256 | `29fc5fa48e61988c08bb5c4d8632fc1b5927e80dbbbda3b2b1fbc6dabc4b2337` |
| blend bytes | `150363405` |
| runtime script SHA-256 | `7f09f514fb33654c60db04113e4d0565c0c74fc741c0dc827402411f9ffbc3c9` |
| reciprocal plan SHA-256 | `2c228040f3f38625ce3e750d8555c070abfcd890281b39f48673e8ee177df9bd`（未变） |
| registry | exact instances `1..218`；`175 + 43 = 218` |
...
## 5. 修复前 fresh Phase 4.3 结果（历史失败证据）
...
### 5.3 junction 开口实现与 `.blend` 内机器证据

runtime 现在显式声明唯一受支持规则：

```text
covered-gallery-underpass / side_branch /
gallery-branch-attachment-side-001 -> path-network-003
```

开口 XY AABB `x=56.2..57.8, y=43.7..46.3` 来自绑定 plan 中该 part 的
`center_m + extent_m + orientation_deg`，没有另建世界坐标白名单。runtime 将
`roadside-vegetation` 按 mesh connectivity 拆为连通分量，要求且只允许恰好一个分量
与包络相交；否则 build fail-closed。删除整个相交分量后再次检查，仍相交则 build 失败。

从新 `.blend` 的 `nv_junction_vegetation_clearance_evidence` 直读：
...
| removed vertices | `16` |
| removed polygons | `28` |
| remaining roadside vegetation | `112 vertices / 196 polygons` |

`terrain-conform-ribbon` 与其它 path children 完全未进入删除逻辑；stable root
`path-network-003`、218 registry、module part identity 均保留。该操作是合成 blockout
的确定性几何清理，不产生真实纹理、测量或重建信任。

### 5.4 修复后 fresh Phase 4.3 结果

| 字段 | 值 |
|---|---|
| probe request SHA-256 | `96b848e607d9b3ffdfe9f3d9b62ec1bb511087742c47839d712f398d107d6751` |
| probe report SHA-256 | `07f3c17e5073a04edcea04db1b9793d6d2fe0fd068f036e50cdc7121c4e7458f` |
| probe script SHA-256 | `97c4fdb00f2aae4f7b474b918e9b1c11c0760c5ecfa70aefda60e4e99be7d2a6` |
| `input_build_id` | `dc6f82c17b06833a2dc709f72c1baa4e5a63a229027a3ebe8debced763dac73d` |
| `input_build_report_sha256` | `7e20184df50bda4a04594a71118c41a0e8d3c75a722b03e2e5524c3542fb97cf` |
| `input_blend_sha256` | `29fc5fa48e61988c08bb5c4d8632fc1b5927e80dbbbda3b2b1fbc6dabc4b2337` |
| `overall_passed` | **true** |
...
| module-module intersection | 15 | 0 |
| topology attachment | 6 | 0 |
| module-environment intersection | 6 | 0 |

原失败项 `covered-gallery-underpass × path-network-003` 现在明确报告
`intersection_count=0`、`intersecting_object_ids=[]`、`passed=true`。route 的最小
clear width/height 仍为约 `1.4m / 2.501m`，没有靠放宽 probe policy 过门。
...
本文件原先交给 GLM 的 junction P0 已由 Codex 独立闭环：TDD、fresh exact-218、`.blend`
机器证据和 fresh Phase 4.3 probe 均完成。GLM 不需要再修改此 junction、probe 阈值或
scene/topology。

接下来可恢复 §3 caller，但 caller 必须消费本节的新 build/report/blend SHA，并重新执行：

1. fresh production preflight；
2. 六层真实 Blender 渲染；
3. post-render quality v2 与 RGB 对比；
4. production render ID / journal 的内容 SHA 绑定。

只有上述 caller 闭环才能解除 req-5 / §3 acceptance；Phase 4.3 通过本身不提升
`modeled-unverified`，也不解决约 `0.198m` 最近相机 pair 或真实照片重建缺口。
...
它们只表达“入口保持开敞、植被在路口确定性退让”的建模意图；实际 acceptance 只来自
§5.3/§5.4 的 runtime、`.blend` 与 fresh probe 机器证据，不来自图片。完整边界见
