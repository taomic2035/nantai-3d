...
1. 为 `gallery-branch-attachment-side-001 ↔ path-network-003` 建模明确的
   **非穿插 junction**（cutout / abutment / transition mesh），同时保留可走连接语义；
   不能简单删 part、制造空隙或在 probe 中忽略交叉。
...
3. 如现有 base path mesh 无法表达 junction，才升级 topology/scene graph；此时必须
   使用真实 edge 连通，禁止孤立 node，并明确告诉 Codex 需要重跑完整 130→175→218。
