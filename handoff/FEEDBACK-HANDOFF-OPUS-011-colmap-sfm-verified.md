...
### Update (2026-07-23): 后续项完成状态

| # | 状态 | 证据 |
|---|---|---|
| 1 | ✅ 完成 | `pipeline/registration.py` `_find_colmap_binary()` 显式发现 `third/colmap/bin/colmap.exe`（行 478-499）；`colmap_register()` 在 COLMAP 不可用时 raise `RuntimeError`（行 558），不静默降级 mock。被 `tests/test_registration.py:376-431` 锁定（4 个测试覆盖 found/not-found/PATH-injection/missing）。 |
| 2 | ✅ 完成 | `pipeline/registration_quality.py` `RegistrationQualityPolicy`（5 阈值，无默认值）+ `build_registration_quality_report()` + `validate_registration_quality()`（重算 SHA + 重算布尔，不信任自报）。REVIEW-CODEX-022 P0.1 修复已关闭（measured 而非 report-authored）。详见 `handoff/FEEDBACK-HANDOFF-GLM-003-registration-quality-design.md`。 |
| 3 | 🔶 部分完成 | `scripts/emit_registration_quality.py` 已就绪（能 emit content-addressed `quality-report.json`，绑定 `registration_json_sha256` + `capture_manifest_sha256` + `policy_canonical_sha256`）。但**仍需真实照片 + accepted SfM** 才能称为 SfM quality accepted；当前只有合成/mock canary。 |
| 4 | ⬜ Codex lane | Studio/Viewer 显示三态 status。未开始。 |

**注意**：`colmap_register()` 本身仍硬编码 `sparse/"0"`（行 589-597），未直接调用 `enumerate_sparse_models()`。这是设计选择——quality report 是独立 artefact，通过 `emit_registration_quality.py` 工具独立产出，不嵌入 `colmap_register()`。`SparseModelEnumeration` 已在 `pipeline/registration_quality.py` 中实现并测试。
