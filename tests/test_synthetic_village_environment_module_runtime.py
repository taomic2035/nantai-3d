...
class TestEnvironmentModuleRequestStableDescriptor:
    """RED->GREEN: _load_request must read the environment-module request through
    ...
    bytes it returns, and refuses to follow symlinks via ``O_NOFOLLOW``.
    """

    @staticmethod
    def _write_canonical_json(
        path: Path, runtime: ModuleType, payload: dict,
    ) -> bytes:
