...
def _make_intrinsics() -> CameraIntrinsics:
    return CameraIntrinsics(width=1920, height=1080, fx=1000.0, fy=1000.0, cx=960.0, cy=540.0)
...
def _write_colmap_model_with_points2d(
    ...
    points = " ".join(f"{i * 0.1:.1f} {i * 0.2:.1f} {i}" for i in range(points2d_count))
    lines = header + points + "\n"
...
class TestSparseModelEnumeration:
    ...
    def test_colmap_points2d_row_not_counted_as_image(self, tmp_path):
        ...
        _write_colmap_model_with_points2d(sparse, image_name="photo.jpg", points2d_count=4)
    ...
    def test_colmap_odd_data_lines_fails_closed(self, tmp_path):
        ...
        (sparse / "images.txt").write_text("1 1 0 0 0 0 0 0 1 photo.jpg\n", encoding="utf-8")
...
class TestThreeStateDecision:
    ...
    def test_invocation_succeeded_false_on_crash(self):
        report, _, _, _, _ = _build_honest_report(registered_count=0, invocation_succeeded=False)
    ...
    def test_quality_accepted_true_when_all_thresholds_met(self):
        report, _, _, _, _ = _build_honest_report(registered_count=18, total_images=20)
    ...
    def test_quality_accepted_false_below_registered_count(self):
        ...
        report, _, _, _, _ = _build_honest_report(registered_count=10, policy=policy)
    ...
    def test_quality_accepted_false_low_model_share(self):
        ...
        registration = _make_registration_result(
            registered_images=[f"img{i:03d}.jpg" for i in range(10)], total_images=20
        )
        reg_bytes = registration.model_dump_json().encode("utf-8")
...
class TestTrainingAllowed:
    ...
    def test_training_allowed_false_without_capture_manifest_sha(self):
        ...
        policy = _make_policy()
        report, _, _, _, _ = _build_honest_report(with_capture_manifest=False, policy=policy)
        assert derive_training_allowed(report, policy) is False
    ...
    def test_training_allowed_true_only_when_all_conditions_met(self):
        ...
        policy = _make_policy()
        report, _, _, _, _ = _build_honest_report(engine="colmap", policy=policy)
...
class TestBuilder:
    ...
    def test_builder_derives_registered_count_from_poses(self):
        report, registration, _, _, _ = _build_honest_report(registered_count=18, total_images=20)
    ...
    def test_builder_derives_total_from_capture_manifest(self):
        report, _, _, manifest_bytes, _ = _build_honest_report(registered_count=18, total_images=20)
    ...
    def test_builder_rejects_registration_bytes_mismatch(self):
        registration_a = _make_registration_result(registered_images=["img000.jpg"], total_images=5)
        registration_b = _make_registration_result(registered_images=["img001.jpg"], total_images=5)
        reg_bytes_b = registration_b.model_dump_json().encode("utf-8")
        with pytest.raises(ValueError, match="does not match"):
            build_registration_quality_report(
                registration=registration_a,
                registration_json_bytes=reg_bytes_b,
                capture_manifest=None,
                capture_manifest_bytes=None,
                policy=_make_policy(),
                sparse_enumeration=_make_sparse_enum(model_image_count=1, total_input_images=5),
                invocation_succeeded=True,
            )
    ...
    def test_builder_rejects_session_total_mismatch(self):
        ...
        registration = _make_registration_result(
            registered_images=[f"img{i:03d}.jpg" for i in range(10)], total_images=15
        )
        reg_bytes = registration.model_dump_json().encode("utf-8")
        manifest_bytes = _make_capture_manifest_bytes(total_images=20)
        from pipeline.studio_revisions import CaptureRevisionManifest
        ...
        with pytest.raises(ValueError, match="session.*total|inconsistent"):
            build_registration_quality_report(
                ...
                capture_manifest_bytes=manifest_bytes,
                policy=_make_policy(),
                sparse_enumeration=_make_sparse_enum(model_image_count=10, total_input_images=20),
                invocation_succeeded=True,
            )
...
class TestAdversarialFailClosed:
    ...
    def test_report_claims_100_but_bytes_say_2_of_20(self):
        ...
        lying_report = RegistrationQualityReport(
            ...
            session_outcomes=(
                SessionQualityOutcome(session_id="s0", registered=100, total=20),
            ),  # lie
    ...
    def test_capture_manifest_sha_bound_but_bytes_missing(self):
        ...
        with pytest.raises(
            ValueError, match="capture_manifest.*bytes|bytes.*required|cannot verify"
        ):
            validate_registration_quality(
                report,
                _make_policy(),
    ...
    def test_session_outcomes_mismatch_with_derived(self):
        ...
        lying_report = RegistrationQualityReport(
            ...
            session_outcomes=(
                SessionQualityOutcome(session_id="s0", registered=18, total=20),
            ),  # lie
            model_enumeration=sparse,
            invocation_succeeded=True,
            quality_accepted=False,
    ...
    def test_engine_mismatch_between_report_and_registration(self):
        ...
        report = RegistrationQualityReport(
            ...
            training_allowed=False,
            rejection_reasons=(
                "registered_count=1 < min_registered_count=10",
...
class TestRoundTrip:
    ...
    def test_tampered_report_file_fails_validation(self, tmp_path):
        ...
        loaded = RegistrationQualityReport.model_validate_json(path.read_text(encoding="utf-8"))
...
def test_colmap_stream_rejects_descriptor_after_drift(
    ...
    with pytest.raises(ValueError, match="changed while being read"):
        registration_quality_module._stream_colmap_text_lines(evidence, label="test")
    assert calls == 2
...
def test_colmap_stream_rejects_symlink(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="bounded regular file"):
        registration_quality_module._stream_colmap_text_lines(link, label="test")
...
def test_colmap_parsers_have_no_bare_read_text() -> None:
    ...
    for func_name in (
        ...
        assert ".read_text(" not in source, f"{func_name} must not use Path.read_text()"
        assert ".read_bytes(" not in source, f"{func_name} must not use Path.read_bytes()"
...
def test_enumerate_sparse_models_error_does_not_leak_absolute_path(
    tmp_path: Path,
) -> None:
    """RED: enumerate_sparse_models ValueError must not leak absolute path.

    The error messages at lines 406, 413, 431 interpolate {sparse_dir},
    which leaks the absolute filesystem path to callers.
    """
    # Scenario 1: directory does not exist
    missing_dir = tmp_path / "private" / "missing-sparse"
    with pytest.raises(ValueError) as captured:
        enumerate_sparse_models(missing_dir, total_input_images=1)
    assert str(tmp_path) not in str(captured.value), f"absolute path leaked: {captured.value}"

    # Scenario 2: empty directory
    empty_dir = tmp_path / "private" / "empty-sparse"
    empty_dir.mkdir(parents=True)
    with pytest.raises(ValueError) as captured:
        enumerate_sparse_models(empty_dir, total_input_images=1)
    assert str(tmp_path) not in str(captured.value), f"absolute path leaked: {captured.value}"

    # Scenario 3: directory with non-numeric subdirectories
    non_numeric_dir = tmp_path / "private" / "non-numeric-sparse"
    non_numeric_dir.mkdir(parents=True)
    (non_numeric_dir / "not-a-number").mkdir()
    with pytest.raises(ValueError) as captured:
        enumerate_sparse_models(non_numeric_dir, total_input_images=1)
    assert str(tmp_path) not in str(captured.value), f"absolute path leaked: {captured.value}"
