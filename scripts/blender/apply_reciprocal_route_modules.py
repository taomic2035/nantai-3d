...
def _load_request(path):
    raw, _sha = _stable_read_and_sha256(
        path, max_bytes=16 * 1024 * 1024, label="reciprocal-route request",
    )
    if not raw:
...
def _build_topology_proxies(request, collection):
    ...
    for module_id, topology_ref, _look_at_m in targets:
        ...
        # Proxies share the same bound material input contract so they cannot
        # render as black vertex-colour fallbacks during audit frames.
        _assign_material_contract(proxy_obj, parent_mesh.data.materials[0])
